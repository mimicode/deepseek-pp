...
- User-created extension data, including memories, skills, project context, project files or source notes, saved snippets, bookmarks, prompt presets, MCP server settings, automation tasks, theme settings, background settings, pet settings, developer settings, and tool execution history.
...
- Send side-panel chat messages or user-started developer test prompts to the official DeepSeek API when the user configures a DeepSeek API Key.
...
- To the official DeepSeek API, when the user configures a DeepSeek API Key and sends a side-panel chat message or user-started developer test prompt.
...
- 用户创建的扩展数据，包括记忆、技能、项目上下文、项目文件或资料说明、保存片段、书签、提示词预设、MCP 服务设置、自动化任务、主题设置、背景设置、宠物设置、开发者设置和工具执行历史。
...
- 用户配置 DeepSeek API Key 时，将侧边栏对话消息或用户主动发起的开发者测试 prompt 发送到 DeepSeek 官方 API。
- 运行用户创建的自动化任务。
...
- 用户配置 DeepSeek API Key 并发送侧边栏对话或用户主动发起开发者测试 prompt 时，传输给 DeepSeek 官方 API。
